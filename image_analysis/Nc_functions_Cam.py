...
def open_images_NC(path):
    ...
    for i, filename in enumerate(files):
        ...
        if counter<5 and filename.endswith('.jpeg'):# and i<len(files)/2:
            #print(filename)
            img = cv2.imread(filename, cv2.IMREAD_GRAYSCALE) #changed by NC to run with other camera images
            counter += 1
            imgs.append(img) 
        elif (counter == 5 or i == (len(files)-1)) and filename.endswith('.jpeg'):# and i<len(files)/2:
            #print('------------')
            
            imgs_avg.extend(temporal_median(imgs, 5))
            imgs = []
...
def open_images_NC2(path):
    ...
    for i, filename in enumerate(files):
        ...
        if filename.endswith('.jpeg'):
            ...
            imgs.append(img) 
        elif (counter == 5 or i == len(files)) and filename.endswith('.jpeg'):
            print('------------')
